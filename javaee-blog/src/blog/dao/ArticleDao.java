...
public interface ArticleDao {
	...
	Map getCommunityColumAndCount(String searchSort);

	List<Article> getAllCommunityArticle();

	List getCommunityVisitRank();

	List<String> getCommunityAllSort();

	List getCommunityArticleByColumn(String column, String value);

	Article getCommunityANearArticle(String time, int less2);

	void addCommunityVisit(int article_id);

	int star_Communityarticle(int id);

	ResultSet getCommunity();
	public List<Article> fengYe(int pageIndex,int pageSize);

	List<Article> CommunityfengYe(int pageIndex, int pageSize);
