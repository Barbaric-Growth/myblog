...
public class CommentDaoImpl implements CommentDao {
	...
	@Override
	public boolean deleteCommunityComment(int comment_id) {
		// TODO Auto-generated method stub

		String sql = "DELETE FROM t_communitycomment WHERE id=" +comment_id ;
		int result = 0;
		try {
			// 文章-1评论
			article_sub_Communitycomemnt(conn, comment_id);
			PreparedStatement ps = conn.prepareStatement(sql);
			result = ps.executeUpdate();
			DBUtils.Close(ps);
		} catch (SQLException e) {
	...
	private void article_sub_Communitycomemnt(Connection conn2, int comment_id) {
		// TODO Auto-generated method stub
		String sql = "SELECT  article_id FROM t_communitycomment WHERE id =" + comment_id;
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			...
			sql = "UPDATE t_community SET COMMENT=COMMENT - 1 WHERE id=" + article_id;
	...
	public boolean addCommunityComment(Comment comment) {
		...
		Connection conn = C3P0Connection.getInstance().getConnection();
		String sql = "INSERT  INTO t_communitycomment VALUES(null,?,?,?,?,?,?)";
		int result = 0;
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, comment.getArticle_id());
			...
			// 文章加1评论
			sql = "UPDATE t_community SET COMMENT = COMMENT+1  WHERE id=" + comment.getArticle_id();
			PreparedStatement ps2 = conn.prepareStatement(sql);
			ps2.executeUpdate();
	...
	@Override
	public List getCommunityComment(int article_id) {
		// TODO Auto-generated method stub
		String sql = "SELECT * FROM t_communitycomment WHERE article_id=? ORDER BY TIME";
		List list = null;
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.setInt(1, article_id);
	...
	public int star_Communitydiss(int id, int star_or_diss) {
		...
		if (star_or_diss == Comment.STAR) {
			sql = "update t_communitycomment set star=star+1 where id=" + id;
		} else if (star_or_diss == Comment.DISS) {
			sql = "update t_communitycomment set diss=diss+1 where id=" + id;
		} else {
			return -1;
		}
		...
		if (star_or_diss == Comment.STAR) {
			sql = "SELECT star FROM t_communitycomment WHERE id = " + id;
		} else if (star_or_diss == Comment.DISS) {
			sql = "SELECT diss FROM t_communitycomment WHERE id = " + id;
		}
