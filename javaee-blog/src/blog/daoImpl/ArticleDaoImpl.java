...
public class ArticleDaoImpl implements ArticleDao {
	...
	public void addCommunityVisit(int article_id) {
		...
		String sql = "update t_community set visit = visit+1 where id = " + article_id;
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.executeUpdate();
		} catch (SQLException e) {
	...
	@Override
	public Article getCommunityANearArticle(String time, int less_or_more) {
		// TODO Auto-generated method stub
		Article article = null;
		String sql = null;
		if (less_or_more == this.LESS) {
			sql = " SELECT  * FROM t_community WHERE TIME< '" + time + "'  ORDER BY TIME DESC ";
		} else if (less_or_more == this.MORE) {
			sql = " SELECT  * FROM t_community WHERE TIME > '" + time + "'  ORDER BY TIME ";
		}
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
	...
	//获取社区分类
	@Override
	public Map getCommunityColumAndCount(String search_column) {
		// TODO Auto-generated method stub
		String sql = " select " + search_column + " ,count(" + search_column + ") as counts  from t_community  group by "
				+ search_column;
		Map map = null;
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
...
//获取社区分类
	@Override
	public List<String> getCommunityAllSort() {
		// TODO Auto-generated method stub
		String sql = " select distinct(sort) from t_community order by sort";
		List list = null;
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
	...
	public Article addArticle(Article a) {
	 ...
	 public void  addCommunityArticle(Article a) {

		String sql = "insert into t_community values(null,?,?,?,?,?,?,?,?)";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setString(1, a.getTitle());
	...
	//获取社区所有文章
	@Override
	public List<Article> getAllCommunityArticle() {
		// TODO Auto-generated method stub
		List<Article> list = new ArrayList();
	...
	public List getCommunityVisitRank() {
		...
		String sql = "SELECT * FROM t_community ORDER BY visit DESC";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
	...
	public List getCommunityArticleByColumn(String column, String value) {
		// TODO Auto-generated method stub
		List<Article> list = null;
		Article at = null;
		String sql = "select * from t_community where " + column + " = ?";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setString(1, value);
	...
	public int star_Communityarticle(int id) {
		String sql = "update t_community set star=star+1 where id=" + id;
		int result = 0;
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ps.executeUpdate();
		...
		sql = "select star from t_community where id=" + id;
		try {
			PreparedStatement ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
	...
	public List<Article> CommunityfengYe(int pageIndex, int pageSize) {
		...
		String sql = "select * from t_community limit ?,?";
		PreparedStatement ps;
		try {
			ps = conn.prepareStatement(sql);
			ps.setInt(1, pageIndex);
