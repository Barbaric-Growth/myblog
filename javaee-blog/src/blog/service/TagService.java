...
public class TagService {
	...
	public List getCommunityTagById(String id) {
		// TODO Auto-generated method stub
		return dao.getCommunityTagByColumn("id", id);
	}
	...
	public Object getCommunityAllTag() {
		// TODO Auto-generated method stub
		return dao.getCommunityAllTag();
	}
	...
	public Map getCommunityTagAndArticle(String tag_name) {
		// TODO Auto-generated method stub
		...
		if (tag_name.equals("all") || StringUtils.isEmpty(tag_name)) {
			// 获取所有不重复的标签
			tag_list = (List<Tag>) dao.getCommunityAllTag();
		} else {
			// 获取这个标签
			tag_list = dao.getCommunityTagByColumn("tag", tag_name);
		}
		...
		// 获取这个标签的所有文章 id
		for (Tag tag : tag_list) {
			List<Tag> list = dao.getCommunityTagByColumn("tag", tag.getTag());
			article_list = new ArrayList();
			// 查询所有文章 id 放在article_list
			for (Tag tag_all : list) {
				// 这个集合只有一个元素
				List<Article> result = as.getCommunityArticle("id", String.valueOf(tag_all.getId()));
				article_list.add(ArticleUtils.cutTime(result.get(0)));
			}
			// 返回标签的内容+标签标记的所有文章集合
			map.put(tag.getTag(), article_list);
